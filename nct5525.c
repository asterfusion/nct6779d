...
static int superio_proc(struct sio_smbus_priv *priv)
{
    ...
    if (!enable_val){
        superio_outb(sioaddr, 0x30, enable_val | 0x01);
    }
    ...
    val_2F = superio_inb(sioaddr, 0x2F);
    val_1A = superio_inb(sioaddr, 0x1A);
    val_1B = superio_inb(sioaddr, 0x1B);
    msleep(1);

    superio_outb(sioaddr, 0x2F, val_2F&0xFD);
    superio_outb(sioaddr, 0x1A, val_1A&0xFC);
    superio_outb(sioaddr, 0x1B, val_1B&0xF9);
